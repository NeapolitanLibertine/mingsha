...
public class Ideology {
    ...
    public int getImageID(){
        if(ImageID < 0){
            ImageID = IMGManager.addIMG("gfx/ideology/"+Image);
        }
        return ImageID;
    }
    public final float getMin_Goods(int nCivID) {
        return Math.max(0.01F, this.MIN_GOODS + GameValues.gvAdministrationPolicy.POLICY_MIN_GOODS[CFG.core.getCiv(nCivID).civGD.policyID] + CFG.religionManager.getReligion(CFG.core.getCiv(nCivID).getReligionID()).MIN_GOODS);
    }
    ...
    public Ideology(String sName, String extraTag, int iR, int iG, int iB, float MIN_GOODS, float MIN_INVESTMENTS, int COST_OF_MOVE, int COST_OF_MOVE_SAME_PROVINCE, int COST_OF_MOVE_OWN_PROVINCE, float RESEARCH_COST, float ACCEPTABLE_TAXATION, int DEFENSE_BONUS, float MILITARY_UPKEEP, float ADMINISTRATION_COST, float ADMINISTRATION_COST_DISTANCE, float INCOME_TAXATION, float INCOME_PRODUCTION, int COST_OF_RECRUIT, int COST_OF_DISBAND, int COST_OF_PLUNDER, int CAN_BECOME_CIVILIZED, int AVAILABLE_SINCE_AGE_ID, float ADMINISTRATION_COST_CAPITAL, float CIVILIZE_TECH_LEVEL, String AI_TYPE, boolean REVOLUTIONARY, int GOV_GROUP_ID, String DESC) {
        ...
        try {
            try {
                this.iCrownImage = new Image(new Texture(FileManager.loadFile("UI/icons/crowns/crown" + extraTag + ".png"), Format.RGBA8888, true), TextureFilter.Linear);
            } catch (GdxRuntimeException var35) {
                this.iCrownImage = new Image(new Texture(FileManager.loadFile("UI/icons/crowns/crown.png"), Format.RGBA8888, true), TextureFilter.Linear);
            }
        } catch (Exception var36) {
            this.iCrownImage = new Image(new Texture(FileManager.loadFile("UI/imageNotFound.png"), Format.RGBA8888, true), TextureFilter.Linear);
        }

        try {
            try {
                this.iCrownVassalImage = new Image(new Texture(FileManager.loadFile("UI/icons/crowns/crown_x" + extraTag + ".png"), Format.RGBA8888, true), TextureFilter.Linear);
            } catch (GdxRuntimeException var33) {
                this.iCrownVassalImage = new Image(new Texture(FileManager.loadFile("UI/icons/crowns/crown_x.png"), Format.RGBA8888, true), TextureFilter.Linear);
            }
        } catch (Exception var34) {
            this.iCrownVassalImage = new Image(new Texture(FileManager.loadFile("UI/imageNotFound.png"), Format.RGBA8888, true), TextureFilter.Linear);
        }

        try {
            try {
                this.iCrownImageScaled = new Image(new Texture(FileManager.loadFile("UI/" + CFG.getResPath() + "crowns/" + "crown" + extraTag + ".png"), Format.RGBA8888, true), TextureFilter.Linear);
            } catch (GdxRuntimeException var31) {
                this.iCrownImageScaled = new Image(new Texture(FileManager.loadFile("UI/" + CFG.getResPath() + "crowns/" + "crown.png"), Format.RGBA8888, true), TextureFilter.Linear);
            }
        } catch (Exception var32) {
            this.iCrownImageScaled = new Image(new Texture(FileManager.loadFile("UI/imageNotFound.png"), Format.RGBA8888, true), TextureFilter.Linear);
        }
