...
public class Menu {
    public List<MenuElemUI> menuElem = new ArrayList();
    ...
    public final void initMenu(TitleM menuTitle, int iPosX, int iPosY, int iWidth, int iHeight, List<MenuElemUI> menuElements, boolean visible, boolean backButton, boolean closeable) {
        ...
        if (backButton) {
            ...
            for(int i = 0; i < this.iMenuElemsSize; ++i) {
                if (menuElements.get(i).getPosY() + menuElements.get(i).getHeightE() > tempMaxY) {
    ...
    public final void updateScrollable() {
        this.iMaxSliderPosY = this.iMaxSliderPositionX = 0;
        ...
        for(int i = 0; i < this.iMenuElemsSize; ++i) {
            ...
            if (this.menuElem.get(i).getPosXE() + this.menuElem.get(i).getWidthE() > this.iMaxSliderPositionX) {
        ...
        this.scrollableY = this.iMaxSliderPosY > this.iHeight;
        if (this.scrollableY) {
    ...
    public void beginClipM(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean sliderMenuIsActive) {
        ...
        Rectangle clipBounds = new Rectangle((float)(this.getPosX() + iTranslateX), (float)(CFG.GAMEHEIGHT - this.getPosY() - iTranslateY), (float)this.getWidthM(), (float)(-this.getHeightM()));
        oSB.flush();
    ...
    public void drawMenuElements(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean sliderMenuIsActive) {
        for(int i = this.iMenuElemsSize - 1; i >= 0; --i) {
            if (this.menuElem.get(i).getVisibleE() && this.menuElem.get(i).getIsInView()) {
                try {
    ...
    public final Image getCloseButtonImage(boolean sliderMenuIsActive) {
        return CFG.menus.getSliderMenuCloseMode() && sliderMenuIsActive ? IMGManager.getIMG(Images.btnhClose) : IMGManager.getIMG(Images.btnClose);
    }
    ...
    public final MenuElemUI getMenuElement(int iID){
        return getMenuElem(iID);
    }
